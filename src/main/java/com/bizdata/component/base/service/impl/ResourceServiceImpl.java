...
public class ResourceServiceImpl implements ResourceService {
	...
	public int insertResource(SysResource sysResource) {
		// 获取父元素
		SysResource parentResource = sysResourceMapper
				.selectByPrimaryKey(sysResource.getParent());
		// 设置新增节点类型
		...
		// 统一增加新节点层级
		sysResource.setLevel(String.valueOf((Integer.parseInt(parentResource
				.getLevel().trim()) + 1)));
	...
	public void deleteResource(Long resourceId) throws Exception {
		...
		SysResource delSysResource = sysResourceMapper
		...
		// 根据parentId查询
		int count = sysResourceMapper.selectCountByParent(delSysResource
				.getParent());
		if (count < 1) {
			// 如果父节点下不存在子节点了，则设置父节点为叶子
			SysResource parentSysResource = sysResourceMapper
					.selectByPrimaryKey(delSysResource.getParent());
			if (null != parentSysResource) {
				parentSysResource.setIsleaf(true);
				sysResourceMapper
						.updateByPrimaryKeySelective(parentSysResource);
			}
		}
	}
	 ...
	 * @return
	 */
	private List<SysResource> getNextLevelResource(SysResource currentNode,
	...
	public String getUserMenus(Set<String> permissions) {
		...
		for (SysResource resource : allResources) {
			...
			for (SysResource menu : allResources) {
				...
				if (menu.getParent() == resource.getId()) {
