	...
	return this.each( function() {
		...
		$(ts).before(grid.hDiv).click(function(e) {
			...
			//else if(!ts.p.treeGrid) {
				...
				ts.p.selrow=null;
				// if(ts.p.multiselect) {ts.p.selarrrow =[];setHeadCheckBox(false);}
				if(ts.p.multiselect || ts.p.treemultiselect) {ts.p.selarrrow =[];setHeadCheckBox(false);}
				ts.p.savedRow = [];
			}
...
$.jgrid.extend({
	...
	setSelection : function(selection,onsr, e) {
		return this.each(function(){
			 ...
			 * 2015年11月25日15:52:46
			 */
			if(!$t.p.multiselect && !$t.p.treemultiselect) {
...
$.jgrid.extend({
	...
	editGridRow : function(rowid, p){
       ...
       * 2015年12月08日14:08:09
       */
			if(rp_ge[$t.p.id].href){
				$(".tooltip").remove()
				if(rowid === "new"){
					location.href = rp_ge[$t.p.id].url
				}else{
					var col = null
          $($t.p.colModel).each(function(i){
            if(rp_ge[$t.p.id].id == this.jsonmap){
              col = i
            }
          })
          var ind = $($t).jqGrid("getInd",rowid)
          var value = $("td:eq("+col+")",$t.rows[ind]).html()
          location.href = rp_ge[$t.p.id].url + "/" + value
				}
				return
			}
...
//									gujianfeng 2015-11-24 modify (去除&& $t.p.datatype !== "local"判断)
//									原先code:if(rp_ge[$t.p.id].reloadAfterSubmit && $t.p.datatype !== "local") {
									if(rp_ge[$t.p.id].reloadAfterSubmit) {
										$($t).trigger("reloadGrid");
									} else {
										if($t.p.treeGrid===true){
...
$.jgrid.extend({
	...
	setTreeGrid : function() {
		return this.each(function (){
			 ...
			 * 2015年11月25日15:52:46
			 */
			if(!$t.p.treemultiselect ) {$.extend($t.p,{treemultiselect: $t.p.multiselect});}
